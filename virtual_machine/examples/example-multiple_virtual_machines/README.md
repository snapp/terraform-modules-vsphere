# Example using vSphere to deploy multiple virtual machines

This folder contains a set of Terraform configuration files (sometimes informally referred to as a "manifest") that serve as examples for how you can use this module to define multiple vSphere virtual machines.
...
To deploy multiple vSphere virtual machines:
...
2. Modify `terraform.tfvars` to contain the values common to all virtual machines you'd like to create.
    > **NOTE**
    > At minimum you must provide `contact`, `datacenter`, `datastore`, `resource_pool`, `template`, and `network`. All other attributes have sensible defaults or are optional.
